...
export class TomSelect {
  ...
  async initialize(selectElement, options = [], config = {}) {
    try {
      ...
      this.logger?.info(
        `🎯 Inicializando TomSelect para: ${selectElement.name || selectElement.id}`
      );
  ...
  _populateSelect(selectElement, options) {
    ...
    this.logger?.info(
      `📝 Select poblado con ${options.length} opciones + placeholder: "${placeholderText}" (valor: "${placeholderValue}")`
    );
  }
  ...
  showValidationError(selectElement, message) {
    try {
      ...
      let errorElement = selectElement.parentElement?.querySelector(`#error_${fieldId}`) ||
  ...
  clearValidationErrors(selectElement) {
    try {
      ...
      const errorElement = selectElement.parentElement?.querySelector(`#error_${fieldId}`) ||
                          selectElement.parentElement?.querySelector(`[data-error-for="${fieldId}"]`) ||
                          selectElement.parentElement?.querySelector(".error-message") ||
  ...
  validateField(instanceKey) {
    ...
    const isRequired =
      selectElement.hasAttribute("required") || selectElement.hasAttribute("data-validation");

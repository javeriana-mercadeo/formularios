...
export class College {
  ...
  async initializeCollegeField() {
    try {
      ...
      this.tomSelectInstance = await this.tomSelectAdapter.initializeByType(collegeElement, [], 'college', {
  ...
  async filterColleges(filters = {}) {
    try {
      const { country = null, department = null, city = null, type = null, sector = null } = filters
  ...
  async searchColleges(searchTerm) {
    try {
      ...
      const searchResults = allColleges.filter(
        college =>
  ...
  _isCustomCollege(value) {
    const allColleges = this.Data.getAllColleges()
    return !allColleges.some(college => college.code === value || college.name === value)
  }

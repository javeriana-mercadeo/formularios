...
class CacheManager {
  ...
  static get EXPIRATION() {
    return (FormConfig.PERSONALIZATION.CACHE_EXPIRATION_HOURS || 24) * 60 * 60 * 1000
  }
  ...
  static get() {
    ...
    try {
      ...
      const isValid = timestamp && Date.now() - parseInt(timestamp) < this.EXPIRATION
  ...
  static set(data) {
    ...
    try {
      ...
      console.log(`📦 Datos guardados en caché por ${FormConfig.PERSONALIZATION.CACHE_EXPIRATION_HOURS || 24} horas`)
    } catch (error) {
...
class Validators {
  ...
  static phone(value) {
    ...
    return clean.length >= FormConfig.VALIDATION.PHONE_MIN_LENGTH && clean.length <= FormConfig.VALIDATION.PHONE_MAX_LENGTH
  }
...
class FormController {
  constructor() {
    ...
    this.validationManager = new ValidationManager(this.formState, this.uiManager)
  }
  ...
  prepareFormForSubmission(form) {
    ...
    const config = FormConfig.PERSONALIZATION.DEBUG_MODE ? this.getDebugConfig() : this.getProdConfig()
...
class FieldManager {
  ...
  initializeCountries() {
    const countrySelect = Utils.getElementById(FormConfig.SELECTORS.FIELDS.COUNTRY)
    ...
    const countries = Object.entries(this.formState.loadedData.locations)
      ...
      .sort((a, b) => (a.value === 'COL' ? -1 : b.value === 'COL' ? 1 : a.text.localeCompare(b.text)))
  ...
  initializePrefixes() {
    const prefixSelect = Utils.getElementById(FormConfig.SELECTORS.FIELDS.PHONE_CODE)
    ...
    const prefixes = this.formState.loadedData.prefixes
      ...
      .sort((a, b) => (a.text.includes('Colombia') ? -1 : b.text.includes('Colombia')))
    ...
    const errorElement = Utils.getElementById(FormConfig.SELECTORS.CONTAINERS.PHONE_ERROR, false)
  ...
  initializeTypeAttendee() {
    const typeSelect = Utils.getElementById(FormConfig.SELECTORS.FIELDS.TYPE_ATTENDEE)
  ...
  initializeAttendanceDay() {
    const daySelect = Utils.getElementById(FormConfig.SELECTORS.FIELDS.ATTENDANCE_DAY)
    if (!daySelect) return
    ...
    if (attendanceDays.length === 0) {
      ...
      const dayRow = daySelect.closest('.jv-form-row') || daySelect.closest('.jv-form-group')
      if (dayRow) {
    ...
    if (attendanceDays.length === 1) {
      ...
      if (dayRow) {
        dayRow.style.display = 'none'
        console.log('📅 Un solo día de asistencia - campo oculto y preseleccionado')
      }
  ...
  initializeAcademicLevel() {
    const levelSelect = Utils.getElementById(FormConfig.SELECTORS.FIELDS.ACADEMIC_LEVEL)
    ...
    if (filteredLevels.length === 1) {
      ...
      console.log(`🔧 Campo nivel académico configurado automáticamente: ${filteredLevels[0].name}`)
      ...
      console.log(`📋 Select de nivel académico inicializado con ${filteredLevels.length} opciones`)
    }
  }
  ...
  loadDepartments() {
    const deptContainer = Utils.getElementById(FormConfig.SELECTORS.CONTAINERS.DEPARTMENT)
    const deptSelect = Utils.getElementById(FormConfig.SELECTORS.FIELDS.DEPARTMENT)
    ...
    if (isColombia) {
      const departments = this.formState.loadedData.locations.COL?.departamentos || []
      const deptOptions = departments.map(dept => ({ value: dept.codigo, text: dept.nombre })).sort((a, b) => a.text.localeCompare(b.text))
  ...
  loadCities() {
    const cityContainer = Utils.getElementById(FormConfig.SELECTORS.CONTAINERS.CITY)
    ...
    if (selectedDept) {
      ...
      const department = this.formState.loadedData.locations.COL?.departamentos.find(d => d.codigo === selectedDept)
  ...
  loadFaculties() {
    const facultyRow = Utils.getElementById(FormConfig.SELECTORS.CONTAINERS.FACULTY_ROW)
    const facultySelect = Utils.getElementById(FormConfig.SELECTORS.FIELDS.FACULTY)
    ...
    if (filteredFaculties.length === 1) {
      this.hideAndSelectSingle(facultySelect, filteredFaculties[0].value, 'FACULTY', facultyRow)
      setTimeout(() => this.loadPrograms(), 100)
    } else if (filteredFaculties.length === 0) {
      this.showNoOptionsMessage(facultySelect, 'No hay facultades disponibles', facultyRow)
    } else {
  ...
  loadPrograms() {
    const programRow = Utils.getElementById(FormConfig.SELECTORS.CONTAINERS.PROGRAM_ROW)
    const programSelect = Utils.getElementById(FormConfig.SELECTORS.FIELDS.PROGRAM)
    ...
    if (programs.length === 1) {
      this.hideAndSelectSingle(programSelect, programs[0].value, 'PROGRAM', programRow)
      setTimeout(() => this.loadPeriods(), 100)
    } else if (programs.length === 0) {
      this.showNoOptionsMessage(programSelect, 'No hay programas disponibles', programRow)
    } else {
  ...
  loadPeriods() {
    const periodRow = Utils.getElementById(FormConfig.SELECTORS.CONTAINERS.PERIOD_ROW)
    const periodSelect = Utils.getElementById(FormConfig.SELECTORS.FIELDS.ADMISSION_PERIOD)
    ...
    const periods = Object.entries(this.formState.loadedData.periods[academicLevel])
  ...
  ensureAcademicFieldsEnabled() {
    ...
    const academicLevelSelect = Utils.getElementById(FormConfig.SELECTORS.FIELDS.ACADEMIC_LEVEL, false)
    const academicLevelRow = Utils.getElementById('academic_level_row', false)
  ...
  levelHasValidContent(levelCode, faculties, programs) {
    ...
    if (faculties?.length > 0) {
      for (const facultyCode in levelData) {
        if (faculties.includes(facultyCode) || faculties.includes(levelData[facultyCode].FacultadCodigo)) {
  ...
  getFilteredFaculties(academicLevel) {
    ...
    if (PROGRAMS?.length > 0) {
      ...
      for (const [facultyCode, facultyData] of Object.entries(levelData)) {
        const matchingPrograms = facultyData.Programas?.filter(program => PROGRAMS.includes(program.Codigo)) || []
      ...
      faculties = FACULTY.map(facultyName => {
        ...
        const found = Object.entries(levelData).find(([code, data]) => code === facultyName || data.FacultadCodigo === facultyName)
  ...
  getFilteredPrograms(academicLevel, faculty) {
    const facultyData = this.formState.loadedData.programs[academicLevel]?.[faculty]
  ...
  handleAcademicFieldsVisibility(typeAttendee) {
    ...
    console.log(`🎓 Tipo de asistente: "${typeAttendee}" - Es aspirante: ${isAspirant}`)

    // Obtener elementos académicos
    const academicLevelRow = Utils.getElementById('academic_level_row', false)
    const facultyRow = Utils.getElementById(FormConfig.SELECTORS.CONTAINERS.FACULTY_ROW, false)
    const programRow = Utils.getElementById(FormConfig.SELECTORS.CONTAINERS.PROGRAM_ROW, false)
    const periodRow = Utils.getElementById(FormConfig.SELECTORS.CONTAINERS.PERIOD_ROW, false)

    const academicLevelSelect = Utils.getElementById(FormConfig.SELECTORS.FIELDS.ACADEMIC_LEVEL, false)
    const facultySelect = Utils.getElementById(FormConfig.SELECTORS.FIELDS.FACULTY, false)
    const programSelect = Utils.getElementById(FormConfig.SELECTORS.FIELDS.PROGRAM, false)
    const periodSelect = Utils.getElementById(FormConfig.SELECTORS.FIELDS.ADMISSION_PERIOD, false)
    ...
    if (isAspirant) {
      ...
      const currentLevel = this.formState.data.academicLevel || (academicLevelSelect && academicLevelSelect.value)
      ...
      ;[academicLevelSelect, facultySelect, programSelect, periodSelect].forEach(select => {
        if (select) {
  ...
  debugAcademicFieldState() {
    ...
    console.log('🔍 Estado del campo académico:', {
      ...
      rowVisible: academicLevelRow ? academicLevelRow.style.display !== 'none' : false,
      ...
      selectOptionsCount: academicLevelSelect ? academicLevelSelect.options.length : 0,
  ...
  updatePrefixDisplay() {
    ...
    const selectedOption = prefixSelect.options[prefixSelect.selectedIndex]
    const container =
      prefixSelect.closest('.jv-phone-code-container') || prefixSelect.closest('.phone-code-wrapper') || prefixSelect.parentElement
...
class ValidationManager {
  ...
  createValidationRules() {
    return [
      ...
      },
      {
        ...
        validator: value => Validators.document(value, this.formState.data.typeDocument),
      ...
      },
      {
        ...
        conditional: () => {
          ...
          const hasValue = this.formState.data.academicLevel && this.formState.data.academicLevel !== ''
      ...
      },
      {
        ...
        conditional: () => {
          ...
          const isVisible = this.isFieldVisibleOrRequired(FormConfig.SELECTORS.FIELDS.FACULTY)
      ...
      },
      {
        ...
        conditional: () => {
          ...
          const isVisible = this.isFieldVisibleOrRequired(FormConfig.SELECTORS.FIELDS.PROGRAM)
      ...
      },
      {
        ...
        conditional: () => {
          ...
          const isVisible = this.isFieldVisibleOrRequired(FormConfig.SELECTORS.FIELDS.ADMISSION_PERIOD)
  ...
  setupPhoneValidation() {
    ...
    const errorElement = Utils.getElementById(FormConfig.SELECTORS.CONTAINERS.PHONE_ERROR)
  ...
  setupAuthorizationValidation() {
    const authRadios = document.querySelectorAll(`input[name="${FormConfig.SELECTORS.FIELDS.DATA_AUTHORIZATION}"]`)
  ...
  validateSingleField(rule) {
    ...
    const element = Utils.getElementById(rule.selector, false)
    const errorElement = Utils.getElementById(rule.errorSelector || `${rule.selector}_error`, false)
  ...
  validatePersonalData() {
    ...
    const personalRules = this.validationRules.filter(rule => !this.shouldSkipField(rule))
  ...
  validateAcademicData() {
    ...
    const attendanceDays = FormConfig.PERSONALIZATION.ATTENDANCE_DAYS || []
    if (attendanceDays.length > 1 && this.isFieldVisible(FormConfig.SELECTORS.FIELDS.ATTENDANCE_DAY)) {
    ...
    if (isAspirant) {
      ...
      const academicLevelField = Utils.getElementById(FormConfig.SELECTORS.FIELDS.ACADEMIC_LEVEL, false)
      if (academicLevelField && academicLevelField.required) {
        const levelValid = Validators.required(this.formState.data.academicLevel)
        console.log('🔍 Nivel académico válido:', levelValid, 'Valor:', this.formState.data.academicLevel)
      ...
      const facultyField = Utils.getElementById(FormConfig.SELECTORS.FIELDS.FACULTY, false)
      if (facultyField && facultyField.required && this.isFieldVisible(FormConfig.SELECTORS.FIELDS.FACULTY)) {
        const facultyValid = Validators.required(this.formState.data.faculty)
        console.log('🔍 Facultad válida:', facultyValid, 'Valor:', this.formState.data.faculty)
      ...
      const programField = Utils.getElementById(FormConfig.SELECTORS.FIELDS.PROGRAM, false)
      if (programField && programField.required && this.isFieldVisible(FormConfig.SELECTORS.FIELDS.PROGRAM)) {
        const programValid = Validators.required(this.formState.data.program)
        console.log('🔍 Programa válido:', programValid, 'Valor:', this.formState.data.program)
      ...
      const periodField = Utils.getElementById(FormConfig.SELECTORS.FIELDS.ADMISSION_PERIOD, false)
      if (periodField && periodField.required && this.isFieldVisible(FormConfig.SELECTORS.FIELDS.ADMISSION_PERIOD)) {
        const periodValid = Validators.required(this.formState.data.admissionPeriod)
        console.log('🔍 Período válido:', periodValid, 'Valor:', this.formState.data.admissionPeriod)
    ...
    if (!authValid) {
      isValid = false
      this.formState.addError('Debes autorizar el tratamiento de datos personales')
    }
  ...
  isFieldVisible(fieldSelector, containerSelector = null) {
    ...
    const fieldRow = field.closest('.jv-form-row') || field.closest('.jv-form-group')
...
class FieldRemovalManager {
  ...
  findFieldContainer(element) {
    ...
    return element.closest('.jv-form-group') || element.closest('.jv-form-row') || element.closest('.form-group') || element.parentElement
  }
  ...
  getDefaultValue(fieldName) {
    const defaults = {
      ...
      dia_asistencia: FormConfig.PERSONALIZATION.ATTENDANCE_DAYS?.[0] || 'Día no configurado',
  ...
  handleSpecialCases(fieldsToRemove) {
    ...
    if (fieldsToRemove.includes('nombre') && fieldsToRemove.includes('apellido')) {
      const nameContainer = Utils.getElementById(FormConfig.SELECTORS.CONTAINERS.NAME, false)
      if (nameContainer) {
    ...
    panel.innerHTML = `
      <div style="margin-bottom: 20px; border-bottom: 1px solid #eee; padding-bottom: 10px;">
        ...
        <p style="margin: 5px 0; color: ${isClean ? 'green' : 'orange'};">
          ${isClean ? `✅ Formulario limpio: ${fieldCount} campos` : `⚠️ Posibles duplicados: ${fieldCount} campos`}
        </p>
      </div>
      <div>
...
class FormSubmissionManager {
  ...
  prepareFormForSubmission(form) {
    ...
    const config = SalesforceConfig.getConfig(FormConfig.PERSONALIZATION.DEBUG_MODE)
  ...
  disableOriginalFields(form) {
    ...
    const originalFields = form.querySelectorAll('input:not([type="hidden"]), select, textarea')
    ...
    console.log(`🔒 ${originalFields.length} campos originales deshabilitados temporalmente`)
  }
  ...
  createSubmissionFields(form, config) {
    ...
    this.createSubmissionField(submissionContainer, 'retURL', FormConfig.URLS.THANK_YOU)
    ...
    if (FormConfig.PERSONALIZATION.DEBUG_MODE) {
      ...
      this.createSubmissionField(submissionContainer, 'debugEmail', FormConfig.PERSONALIZATION.EMAIL_DEBUG)
      fieldsCreated += 2
      console.log(`   🐛 Campos de debug agregados: debug=1, debugEmail=${FormConfig.PERSONALIZATION.EMAIL_DEBUG}`)
    }

    // Mapear datos del estado a campos de Salesforce
    Object.entries(config.FIELD_MAPPING).forEach(([stateKey, salesforceField]) => {
  ...
  shouldIncludeField(stateKey, value) {
    const alwaysInclude = ['originCandidate', 'sourceAuthorization', 'school', 'originRequest']

    return alwaysInclude.includes(stateKey) || (value !== null && value !== undefined && value !== '')
  }
  ...
  handleSubmissionError(form) {
    ...
    this.uiManager.showErrors(['Ocurrió un error al enviar el formulario. Por favor, inténtalo de nuevo.'])
  }
...
if (typeof FormController !== 'undefined') {
  ...
  FormController.prototype.initSubmissionManager = function () {
    this.submissionManager = new FormSubmissionManager(this.formState, this.uiManager)
  }
